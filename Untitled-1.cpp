...
int main(void) {
    InitWindow(SCREEN_W, SCREEN_H, "Snakebird - Mejorado");
    SetTargetFPS(60);
    srand(time(NULL));
    ...
    while (!WindowShouldClose()) {
        ...
        // Movimiento más natural con aceleración lateral
        ...
        vel.x += move * accel * dt;
        if (move == 0) vel.x = Lerp(vel.x, 0, 5 * dt);
        if (vel.x > maxSpeed) vel.x = maxSpeed;
        if (vel.x < -maxSpeed) vel.x = -maxSpeed;

        segments[0].pos.x += vel.x * dt;
        ...
        // Piso para que no se caiga
        if (segments[0].pos.y > SCREEN_H - 48) {
            segments[0].pos.y = SCREEN_H - 48;
            vel.y = 0;
            onGround = true;
        }

        // Movimiento de segmentos (seguimiento)
        ...
        for (int b = 0; b < bananaCount; b++) {
            ...
            if (CheckCollisionCircleRec(bananas[b].pos, bananas[b].radius, headRect)) {
                ...
                SpawnRandomBanana(); // Nueva banana al comer una
            }
        }
        ...
        BeginDrawing();
            ...
            for (int i = 0; i < platformCount; i++) {
                ...
                DrawRectangleRec(r, (Color){230, 230, 255, 255});
            ...
            DrawText(TextFormat("Bananas: %d  Segmentos: %d", bananaCount, segCount), 10, 10, 20, BLACK);
            DrawText("Flechas o A/D = mover | W/Space = saltar | R = reiniciar", 10, SCREEN_H - 28, 16, BLACK);
        EndDrawing();
    }
...
void ResetGame(void) {
    ...
    SpawnPlatform(0, SCREEN_H - 40, SCREEN_W, 40); // piso principal
...
void SpawnRandomBanana(void) {
    if (bananaCount >= MAX_BANANAS) return;
    float x = 100 + rand() % (SCREEN_W - 200);
    float y = 100 + rand() % (SCREEN_H - 300);
    SpawnBanana(x, y);
}
